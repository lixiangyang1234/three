...
//  Community
//
//  Created by notbadboy on 14-7-19.
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            ...
            _topScrollView.frame = CGRectMake(0, 40,
...
- (void)createNameButtons
{
    ...
    //_topScrollView .backgroundColor =[UIColor cyanColor];
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin+textSize.width+30;
# if  0
//        设置按钮尺寸
//        [button setFrame:CGRectMake(xOffset,0,
//                                    textSize.width+130, kHeightOfTopScrollView)];
#endif
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,0,
                                    kWidth/3-3, kHeightOfTopScrollView)];
...
//        button.alpha =0.7;
        ...
        if (self.isNeedIcon) {
            ...
            imgView.tag=110+i;
            if (i == 0) {
                imgView.frame = CGRectMake(25, 15, 16, 15);
                 imgView.image = [UIImage imageNamed:@"slide_friend_btn_pressed.png"];
            }else{
                imgView.frame = CGRectMake(165, 15, 15, 16);
                 imgView.image = [UIImage imageNamed:@"slide_linli_btn.png"];
            }
...
# if  0
    UIImageView *aView = [[UIImageView alloc] initWithFrame:CGRectMake(0,43,500,1)];
    aView.backgroundColor=UIColorFromRGB(0x1bbc9b);
...
# if  0
                ...
                for(UIImageView *sv in _topScrollView.subviews)
                {
                    if (sv.tag == 110)
                    {
                        sv.image = [UIImage imageNamed:@"slide_friend_btn_pressed.png"];
                     }else if(sv.tag == 111)
                        sv.image = [UIImage imageNamed:@"slide_linli_btn.png"];
                }
            ...
            }else
            {
                for(UIImageView *sv in _topScrollView.subviews)
                {
                    if (sv.tag == 110)
                    {
                        sv.image = [UIImage imageNamed:@"slide_friend_btn.png"];
                     }else if(sv.tag == 111)
                    {
                        sv.image = [UIImage imageNamed:@"slide_linli_btn_pressed.png"];
                    }
                }
# if  0
